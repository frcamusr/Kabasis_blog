...
from django.http import JsonResponse
from .models import Curso, UnidadCurso, Progreso, QuizContent, Video, ProgresoUnidad
from django.contrib import messages
from django.contrib.auth.decorators import login_required
from django.db.models import Max
from django.urls import reverse
...
def ver_curso(request, id):
    ...
    if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
        if unidad_siguiente and total_contenidos_unidad == contenido_avance:
            response_data = {
                'unidad_id': unidad_siguiente.id,
                'aprobada': True,
                'nueva_imagen': unidad_siguiente.imagen.url if unidad_siguiente.imagen else static('ruta/a/noaprobado.jpg')
            }
            return JsonResponse(response_data)
        else:
            response_data = {'unidad_id': None}
            return JsonResponse(response_data)
